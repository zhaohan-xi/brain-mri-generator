...
class ImageFolder(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with torch.no_grad():
            seq_tgt = model.encode_text(seq_tgt)  # TODO, change into a valid encoder

        # print(patch_src_hr, patch_tgt_hr)
        if patch_src_hr.endswith("npy"):
            img_vol_src_hr = np.load(patch_src_hr.strip(), allow_pickle=True)  
        elif patch_src_hr.endswith("nii.gz"):
             img_vol_src_hr = nib.load(patch_src_hr.strip()).get_fdata()
        else:
            raise NotImplementedError("not support other img format")
        
        if patch_tgt_hr.endswith("npy"):
            img_vol_tgt_hr = np.load(patch_tgt_hr.strip(), allow_pickle=True)
        elif patch_tgt_hr.endswith("nii.gz"):
            img_vol_tgt_hr = nib.load(patch_tgt_hr.strip()).get_fdata()
        else:
            raise NotImplementedError("not support other img format")
        
        # print(img_vol_src_hr.shape, img_vol_tgt_hr.shape)
