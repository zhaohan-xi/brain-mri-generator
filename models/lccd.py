...
class LCCD(nn.Module):
    ...
    # test forward
    def generation(self, src_lr, coord_hr, prompt_tgt):
        N, K = coord_hr.shape[:2]
        ...
        # print("N, K", N, K)
        # print("feat_src_lr.shape", feat_src_lr.shape)
        # print("feat_src_lr_tgt.shape", feat_src_lr_tgt.shape)
        # print("vector_src_tgt.shape", vector_src_tgt.shape)
        # print("vector_src_tgt_with_coord.shape", vector_src_tgt_with_coord.shape)
        # print("pre_src_tgt.shape", pre_src_tgt.shape)
        
        # N, K 1 216000
        # feat_src_lr.shape torch.Size([1, 768, 60, 60, 60])
        # feat_src_lr_tgt.shape torch.Size([1, 768, 60, 60, 60])
        # vector_src_tgt.shape torch.Size([1, 216000, 768])
        # vector_src_tgt_with_coord.shape torch.Size([1, 216000, 771])
        # pre_src_tgt.shape torch.Size([1, 216000, 1])
        return pre_src_tgt
