...
def eval_psnr(loader, model):
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            ...
            # print("generated", pre_src_tgt.shape, pre_tgt_src.shape)
            # print("targeted", batch['coord_hr'].shape, batch['seq_src'].shape, batch['seq_tgt'].shape)
...
def write_img(out_img, out_path, ref_path, new_spacing=None):
    img_ref = sitk.ReadImage(ref_path)
    # vol_stack = torch.stack(vol, dim=0)  # original list of tensors
    # print("-"*100)
    # print('type(img_ref)', type(img_ref))
    # print("torch.stack(vol, dim=0).shape", vol_stack.shape)
    # .reshape(coord_size)
    img = sitk.GetImageFromArray(out_img.cpu().numpy())
